...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // add an onclick to each square in your grid
        squares[i].onclick = function(){
            ...
            //if the square below your current square is taken, you cannot go on top of it
            if(squares[index + 7].classList.contains('taken')){
                ...
                // if the square below current square is not taken, you cannot go there
                }else alert('canot go there')
            }
    }) (i)
    ...
    function checkBoard(){
        ...
        //now take tha values in each winning array and plug them into the squares
        for(let y = 0; y < winningArrays.length; y++){
            ...
            // now check the arrays to see if they all have the calls of player-one
            if(square1.classList.contains('player-one') &&
            square2.classList.contains('player-one') &&
            square3.classList.contains('player-one') &&
            square4.classList.contains('player-one')) {
                //if they do player one displays as the winner
                result.innerHTML = "Player 1 Wins!"
            } 
            // now check if they contain the class of player two
            else if(square1.classList.contains('player-two') &&
            square2.classList.contains('player-two') &&
            square3.classList.contains('player-two') &&
            square4.classList.contains('player-two')) {
                //if they do player two displays as the winner
